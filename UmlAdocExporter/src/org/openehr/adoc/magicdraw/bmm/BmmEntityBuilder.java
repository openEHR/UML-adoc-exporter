...
import org.openehr.adoc.magicdraw.imm.ImmFeature;
 ...
 * @author Thomas Beale
 */
public abstract class BmmEntityBuilder<T> extends AmmEntityBuilder<T> {

    protected BmmEntityBuilder(org.openehr.adoc.magicdraw.Formatter formatter, Function<String, Class> getUMLClassByQualifiedName) {
        super(formatter, getUMLClassByQualifiedName);
    }

    // Add class- or routine-level constraints
    protected void addConstraints (AmmClass ammClass, Collection<Constraint> umlConstraints) {
        for (Constraint umlConstraint : umlConstraints) {
            ammClass.addConstraint(new BmmConstraintAdapter(new BmmAssertion(extractUmlonstraintText(umlConstraint), extractUmlonstraintTag(umlConstraint))));
        }
    }
     ...
     * @param ammClass BMM class being built.
    ...
    protected <C extends AmmClass> void addAttribute(C ammClass, Property umlProperty, FeatureDefinitionStatus attrStatus) {
        ...
        BmmPropertyAdapter bmmPropertyAdapter = new BmmPropertyAdapter(new BmmProperty<>());

        bmmPropertyAdapter.setDocumentation(getUmlDocumentation(umlProperty, formatter));
        bmmPropertyAdapter.setCardinality(umlProperty.getLower(), umlProperty.getUpper());
        bmmPropertyAdapter.setStatus(attrStatus);
        ...
        ammClass.addAttribute(immClassFeature);
    }

    /**
     * Build a ClassFeatureInfo for operation, and append it to the features list so far built.
     * @param ammClass List of class features so far built.
     ...
     */
    protected void setHierarchy (String umlQualifiedName, int pkgDepth, BmmClassAdapter bmmClassAdapter) {
        String[] parts = umlQualifiedName.split ("::");
        int depth = Math.min(parts.length, pkgDepth);
        if (parts.length > depth) {
            bmmClassAdapter.setSpecComponent(parts[0]);
            bmmClassAdapter.setClassPackage(parts[depth]);

            StringBuilder sb = new StringBuilder();
            for (int i = 1; i < depth; i++)
                sb.append('.').append (parts[i]);
            bmmClassAdapter.setComponentPackage(sb.substring(1));
        }
    }
